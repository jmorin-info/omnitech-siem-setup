...
mk_a "OMNI - Exécution proxy via InstallUtil"                        "alert_tag:installutil_proxy"  "$SYS"
mk_a "OMNI - Application Office lance un interpreteur (macro)"        "alert_tag:office_spawn_shell" "$SYS"
mk_a "OMNI - Téléchargement via certutil (LOLBin)"                   "alert_tag:certutil_download" "$SYS"
mk_a "OMNI - Effacement de journal d'événements (wevtutil)"          "alert_tag:wevtutil_clear"    "$SYS"
mk_a "OMNI - Suppression du journal USN (fsutil, anti-forensique)"   "alert_tag:fsutil_usn"        "$SYS"
mk_a "OMNI - Exécution via WMI (process call create)"               "alert_tag:wmic_exec"         "$SYS"
echo
