...
pages = [
 ...
 {"title": "Direction", "streams": S(WINSEC, SYSMON, WINOTH, FORTI, M365, INT), "widgets": [
   ...
   KPI("Incidents critiques (corrélés XDR)", "event_source:xdr_incident AND severity:critical", 7, 3,
       card="host", range=86400, dir="LOWER",
       desc="Entités avec un incident corrélé CRITIQUE (oms-xdr : kill-chain multi-signaux). Le signal le plus actionnable du tableau. Détail : onglets OMS-XDR / Incidents."),
   ...
   # --- Matrice ATT&CK : couverture par tactique (ajout audit 28/06) ---
   W("Matrice ATT&CK — détections par tactique (30j)", "_exists_:mitre_tactic", "table", 1, 20, 12, 5,
     pivot="mitre_tactic", limit=14, range=D30,
     metrics=["count", ("card", "mitre_technique", "Techniques"), ("card", "host", "Hôtes"), ("card", "user", "Comptes")],
     desc="Couverture ATT&CK : volume de détections par TACTIQUE sur 30j (les phases de la kill-chain). Une tactique absente/à 0 = angle mort de détection à combler. Détail par technique : onglet OMS-XDR."),
 ]},
 ...
 {"title": "M365", "streams": S(M365), "widgets": [
   ...
   # --- Entra ID : abus de privileges / backdoor cloud (94-entra.sh + correlation 96) ---
   KPI("Secret/cert sur app (backdoor)", "alert_tag:m365_app_credential_add", 1, 26, dir="LOWER", range=D30,
       desc="Ajout d'un secret/certificat sur une app/service principal Entra = porte derobee cloud persistante (T1098.001). Hors provisioning declare = critique."),
   KPI("Conditional Access modifie", "alert_tag:m365_ca_change", 4, 26, dir="LOWER", range=D30,
       desc="Creation/modif d'une politique Conditional Access = affaiblissement possible de la MFA / des controles d'acces (T1556)."),
   KPI("Autorite anormale (hors baseline)", "alert_tag:m365_authority_drift", 7, 26, dir="LOWER", range=D30,
       desc="Action privilegiee M365 (secret app / CA / role) par un compte HORS baseline (jmorin + service principals connus) = anomalie d'autorite."),
   KPI("Role privilegie attribue", "alert_tag:m365_role", 10, 26, dir="LOWER", range=D30,
       desc="Attribution d'un role a privileges dans Entra. A valider par un ticket de change."),
   W("Entra - actions privilegiees / backdoor (detail, 30j)",
     "alert_tag:(m365_app_credential_add OR m365_ca_change OR m365_authority_drift OR m365_role)", "messages", 1, 28, 12, 4,
     range=D30, show_message=False,
     fields=["timestamp", "user", "event_action", "event_category", "src_ip", "src_country", "alert_tag"],
     desc="Detail des actions privilegiees / persistance cloud sur le tenant Entra."),
 ]},
 ...
 # ----- INCIDENTS : correlation XDR (oms-xdr / xdr_incident) -----------------
 # Moteur UNIQUE depuis 28/06 (omni-incident-correlate deprecie + console deja sur
 # xdr_incident). Incidents correles par REGLE : signaux multiples -> kill-chain nommee,
 # MITRE, pont user<->host, narration LLM. Champs : severity, host, _tactic, _mitre, _signals.
 {"title": "Incidents", "streams": S(INT), "widgets": [
   KPI("Incidents CRITIQUES (en cours)", "event_source:xdr_incident AND severity:critical", 1,
       range=86400, dir="LOWER",
       desc="Incidents corrélés CRITIQUES (oms-xdr : >=2 signaux ou signal haute-fidélité). À traiter en priorité absolue."),
   KPI("Incidents élevés", "event_source:xdr_incident AND severity:high", 4,
       range=86400, dir="LOWER",
       desc="Incidents corrélés de sévérité élevée."),
   KPI("Entités en incident", "event_source:xdr_incident", 7, card="host", range=86400, dir="LOWER",
       desc="Nombre d'entités (hôtes/comptes) impliquées dans un incident corrélé sur la fenêtre."),
   KPI("Incidents corrélés (24h)", "event_source:xdr_incident", 10, range=86400, dir="LOWER",
       desc="Volume total d'incidents corrélés par oms-xdr (toutes sévérités) sur 24h."),
   W("Incidents par entité", "event_source:xdr_incident", "table", 1, 3, 8, 5,
     pivot="host", limit=25, range=86400, metrics=["count"],
     desc="Entités classées par nombre d'incidents corrélés. Le détail narratif (LLM) est en dessous."),
   W("Répartition par sévérité", "event_source:xdr_incident", "pie", 9, 3, 4, 5, pivot="severity", range=86400),
   W("Récits d'attaque - narration LLM (détail)", "event_source:xdr_incident", "messages", 1, 8, 12, 7,
     range=86400, show_message=True,
     fields=["severity", "_entities", "_mitre", "_tactic", "_signals"],
     desc="Chaque incident raconte : entités, techniques/tactiques ATT&CK, signaux corrélés, et une narration analyste (LLM oms-xdr). Transforme des alertes éparses en histoire exploitable."),
 ]},
